...
const SparkleIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M12 2L13.5 9L20 10.5L13.5 12L12 19L10.5 12L4 10.5L10.5 9L12 2Z" fill="#0A0A0A"/>
    <path d="M19 2L19.75 5.25L23 6L19.75 6.75L19 10L18.25 6.75L15 6L18.25 5.25L19 2Z" fill="#0A0A0A" opacity="0.5"/>
  </svg>
)
...
const TaglineLines = () => (
  <svg width="60" height="20" viewBox="0 0 60 20" fill="none">
    <line x1="0" y1="10" x2="60" y2="10" stroke="#0A0A0A" strokeWidth="1" strokeOpacity="0.3"/>
    <line x1="10" y1="5" x2="50" y2="5" stroke="#0A0A0A" strokeWidth="1" strokeOpacity="0.15"/>
    <line x1="20" y1="15" x2="40" y2="15" stroke="#0A0A0A" strokeWidth="1" strokeOpacity="0.1"/>
  </svg>
)
...
export default function PortfolioCanvas() {
  ...
  useEffect(() => {
    ...
    const startGhostCursor = (wordEl: HTMLElement, homeCX: number, homeCY: number) => {
      gsap.killTweensOf(ghostCursor); clearTyping(); gsap.set(ghostCursor, { opacity: 0 })
      ...
      gsap.set(ghostCursor, { x: corners[entryIdx].x, y: corners[entryIdx].y })
      ...
      gsap.delayedCall(0.1, () => {
        ...
        gsap.delayedCall(0.2, () => {
          if (ghostGenRef.current !== gen) return
          gsap.to(ghostCursor, {
            x: droppedX, y: droppedY, duration: 1.2, ease: 'power2.inOut',
            onComplete() {
              if (ghostGenRef.current !== gen) return
              highlightBorder(wordEl)
              gsap.to(wordEl, { x: 0, y: 0, duration: 1.2, ease: 'power2.out' })
              gsap.to(ghostCursor, {
                x: homeCX, y: homeCY, duration: 1.2, ease: 'power2.out',
                onComplete() {
                  if (ghostGenRef.current !== gen) return
                  revertBorder(wordEl)
                  gsap.delayedCall(0.6, () => {
                    ...
                    typeMessage(message, () => {
                      gsap.delayedCall(1.5, () => {
                        ...
                        gsap.to(ghostCursor, {
                          x: exitCorners[exitIdx].x, y: exitCorners[exitIdx].y,
                          duration: 1.0, ease: 'power2.inOut',
                          onComplete() {
                            if (ghostGenRef.current !== gen) return
                            gsap.set(ghostCursor, { opacity: 0 }); clearTyping()
    ...
    const makeHeroDraggable = (wordEl: HTMLElement) => {
      ...
      Draggable.create(wordEl, {
        ...
        onDragStart() {
          ...
          gsap.killTweensOf(ghostCursor); gsap.set(ghostCursor, { opacity: 0 })
    ...
    if (polaroidRef.current) {
      ...
      Draggable.create(polaroid, {
        ...
        onDragStart() { polDragDist = 0; polStartX = this.x; polStartY = this.y; gsap.to(polaroid, { rotation: -3, duration: 0.2 }) },
        onDrag() {
        ...
        onDragEnd() { gsap.to(polaroid, { rotation: -10, duration: 0.4, ease: 'elastic.out(1,0.6)' }) }
      })
    ...
    if (d1 && d2 && d3 && d4 && deckWrapper) {
      ...
      Draggable.create(deck, {
        type: 'x,y', inertia: true,
        onDragStart() { anyDragActiveRef.current = true; deckDragDist = 0; deckStartX = this.x; deckStartY = this.y; gsap.to(deck, { rotation: 3, duration: 0.2 }) },
        onDrag() {
          deckDragDist = Math.sqrt(Math.pow(this.x - deckStartX, 2) + Math.pow(this.y - deckStartY, 2))
          gsap.set(cursorArrow, { x: this.pointerX, y: this.pointerY }); gsap.set(cursorTag, { x: this.pointerX + 22, y: this.pointerY + 16 })
        },
        onDragEnd() { anyDragActiveRef.current = false; gsap.to(deck, { rotation: 0, duration: 0.4, ease: 'elastic.out(1,0.6)' }) }
      })
    ...
    if (contactRef.current) {
      ...
      Draggable.create(c, {
        type: 'x,y', inertia: true,
        onDragStart() { cDragDist = 0; cStartX = this.x; cStartY = this.y; gsap.to(c, { rotation: 5, duration: 0.2 }) },
        onDrag() {
          cDragDist = Math.sqrt(Math.pow(this.x - cStartX, 2) + Math.pow(this.y - cStartY, 2))
          gsap.set(cursorArrow, { x: this.pointerX, y: this.pointerY }); gsap.set(cursorTag, { x: this.pointerX + 22, y: this.pointerY + 16 })
        },
        onDragEnd() { gsap.to(c, { rotation: 10, duration: 0.4, ease: 'elastic.out(1,0.6)' }) }
      })
    ...
    if (gpStickerRef.current) {
      ...
      Draggable.create(gp, {
        type: 'x,y', inertia: true,
        onDragStart() { gpDragDist = 0; gpStartX = this.x; gpStartY = this.y; gsap.to(gp, { rotation: 2, duration: 0.2 }) },
        onDrag() {
          gpDragDist = Math.sqrt(Math.pow(this.x - gpStartX, 2) + Math.pow(this.y - gpStartY, 2))
          gsap.set(cursorArrow, { x: this.pointerX, y: this.pointerY }); gsap.set(cursorTag, { x: this.pointerX + 22, y: this.pointerY + 16 })
        },
        onDragEnd() { gsap.to(gp, { rotation: 6, duration: 0.4, ease: 'elastic.out(1,0.6)' }) }
      })
    ...
    <>
      ...
      <div id="canvas" ref={canvasRef}>
        ...
        <div id="hero-tagline">
          ...
          <span className="tagline-svg-right"><TaglineLines /></span>
        </div>

        {/* Polaroid */}
        <a id="polaroid" ref={polaroidRef} href="/about" aria-label="About Me">
          {/* Replace src with your actual photo */}
          <div id="polaroid-photo" style={{ width: 220, height: 220, borderRadius: 2, background: '#C8C5BE', display: 'block' }} />
          ...
          <div className="deck-wrapper">
            <div className="project-card" id="deck-card-4" ref={deckCard4Ref as React.RefObject<HTMLDivElement>}>
              <div className="card-image">
                <div className="card-image-placeholder" style={{ background: '#B0ADA6' }} />
            ...
            <div className="project-card" id="deck-card-3" ref={deckCard3Ref as React.RefObject<HTMLDivElement>}>
              <div className="card-image">
                <div className="card-image-placeholder" style={{ background: '#B8B5AE' }} />
            ...
            <div className="project-card" id="deck-card-2" ref={deckCard2Ref as React.RefObject<HTMLDivElement>}>
              <div className="card-image">
                <div className="card-image-placeholder" style={{ background: '#C0BDB6' }} />
            ...
            <a className="project-card" id="deck-card-1" ref={deckCard1Ref} href="/work" aria-label="View Work">
              <div className="card-image">
                <div className="card-image-placeholder" style={{ background: '#C8C5BE' }} />
        ...
        <a id="contact-sticker" ref={contactRef} href="/contact" aria-label="Contact Dhairya"
          ...
          {/* Replace with actual sticker image */}
          <div style={{ width: 280, height: 120, background: '#0A0A0A', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <span style={{ color: 'white', fontFamily: 'var(--font-space-grotesk)', fontSize: 20, fontWeight: 700 }}>Contact Me →</span>
          </div>
        </a>
        ...
        <div id="good-problems-sticker" ref={gpStickerRef} role="button" tabIndex={0} aria-label="Good Problems"
             ...
             onMouseLeave={() => setCursorLabel('You')}>
          {/* Replace with actual sticker image */}
          <div style={{ width: 280, height: 120, background: '#5B4CDB', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <span style={{ color: 'white', fontFamily: 'var(--font-space-grotesk)', fontSize: 18, fontWeight: 700 }}>Good Problems ✦</span>
          </div>
        </div>
      </div>
        ...
        <div id="ai-panel-header">
          ...
          <div className="ai-header-right">
            <button className="ai-header-icon-btn" onClick={handleAiReset} aria-label="Reset chat">
              <span style={{ fontSize: 16, color: '#0A0A0A', opacity: 0.5 }}>↺</span>
            </button>
